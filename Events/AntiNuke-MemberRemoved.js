...
module.exports = {
  ...
  async execute(m) {
    ...
    const LIMIT = data.antinukesettings.members_kicked_before_time_before_quarantine;

    const DIFF = data.antinukesettings.members_kicked_time * 1000
    const TIME = data.antinukesettings.members_kicked_time * 1000
    if (usersMap.has(member.id)) {
      
      const userData = usersMap.get(member.id);
      userData.members_kicked.push(member)
      const {
        lastKickedMember,
        timer
      } = userData;
      const difference = Date.now() - lastKickedMember;
      ...
      if (difference > DIFF) {
        ...
        userData.lastKickedMember = Date.now();
        ...
        if (parseInt(memberCount) >= LIMIT) {
          ...
          if (quarantined !== true) {
            if (member.user.bot) {
              ...
              guild.bans.create(member, {
                reason: `Kicked ${LIMIT} members before ${TIME/1000} seconds`
              }).then(async () => {
              ...
              functions.sendbotlogs(guild, {
                title: `Anti-Nuke Member Kick`,
                description: `Quarantine Member Does Not Exist/Not Set, Trying To Kick Member Instead...\nTrying To Quarantine A User: ${member.user.tag}`,
                color: "DARK_BUT_NOT_BLACK"
              })
              member.kick(`Kicked ${LIMIT} members before ${TIME/1000} seconds`).then(async () => {
                await functions.dont_save_members(member)
                functions.sendbotlogs(guild, {
                  title: `Anti-Nuke Member Kick`,
          ...
          return functions.sendbotlogs(guild, {
            title: `Anti-Nuke Member Kick`,
            description: `Quarantined User: ${member.user.tag}\nKicked ${LIMIT} members before ${TIME/1000} seconds`,
            color: "DARK_BUT_NOT_BLACK"
          })
      ...
      usersMap.set(member.id, {
        memberCount: 1,
        lastKickedMember: Date.now(),
        timer: fn,
        members_kicked: [member]
      });
      let {
        memberCount
