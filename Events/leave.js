...
module.exports = {
  ...
  async execute(member) {
    ...
    if (!data || data.leave_channel == null || !find_channel(data.leave_channel) || (!data.leave_message.content)) return;
    let channel = find_channel(data.leave_channel)
    let lmsg = data.leave_message
    if (lmsg.embed.title == null && lmsg.embed.description == null && lmsg.embed_enabled == true) return channel.send({
      content: "Leave Embed Title And Description Missing, One Is Required, Or Just Disable Leave Embed"
    })
