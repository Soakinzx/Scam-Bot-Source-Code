  ...
  async execute(channel) {
    ...
    if (!guild.me.permissions.has("VIEW_AUDIT_LOG")) {
      return functions.sendbotlogs(guild, {
          title: `Anti-Nuke Channel Delete`,
    ...
    const LIMIT = data.antinukesettings.channels_deleted_before_time_before_quarantine;

    const DIFF = data.antinukesettings.channels_deleted_time * 1000
    const TIME = data.antinukesettings.channels_deleted_time * 1000
    if (usersMap.has(member.id)) {
      
      const userData = usersMap.get(member.id);
      userData.channels_deleted.push(channel)
      const {
        lastDeletedChannel,
        timer
      } = userData;
      const difference = Date.now() - lastDeletedChannel;
      ...
      if (difference > DIFF) {
        ...
        userData.lastDeletedChannel = Date.now();
        ...
        if (parseInt(chnlCount) >= LIMIT) {
          ...
          if (quarantined !== true) {
            if (member.user.bot) {
              ...
              guild.bans.create(member, {
                reason: `Deleted ${LIMIT} channels before ${TIME/1000} seconds`
              }).then(async () => {
              ...
              member.kick(`Deleted ${LIMIT} channels before ${TIME/1000} seconds`).then(async () => {
                await functions.dont_save_roles(member)
                functions.sendbotlogs(guild, {
                  title: `Anti-Nuke Channel Delete`,
          ...
          return functions.sendbotlogs(guild, {
            title: `Anti-Nuke Channel Delete`,
            description: `Quarantined User: ${member.user.tag}\nDeleted ${LIMIT} channels before ${TIME/1000} seconds`,
            color: "DARK_BUT_NOT_BLACK"
          })
      ...
      usersMap.set(member.id, {
        chnlCount: 1,
        lastDeletedChannel: Date.now(),
        timer: fn,
        channels_deleted: [channel]
      });
      let {
        chnlCount
