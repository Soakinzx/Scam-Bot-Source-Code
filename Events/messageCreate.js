  ...
  async execute(message) {
    ...
    if (message.content.startsWith(prefix)) {
      ...
      if(guild_data && guild_data.blacklisted.includes(message.member.id)){
        try {
          return member.send({content: `You are currently blacklisted from using commands in \`${message.guild.name}\``})
        } catch(err){
          message.reply({content: `You are currently blacklisted from using commands in \`${message.guild.name}\``})
        }
        return;
      }
