...
function format(data){
        ...
        if (!data || data.leave_channel == null || !find_channel(data.leave_channel)) return;
        let channel = find_channel(data.leave_channel)
        let lmsg = data.leave_message
        if(lmsg.embed){
          if(lmsg.content){
            channel.send({
            content: format(lmsg.content),
            embeds: [format(lmsg.embed)]
        })
          } else {
            channel.send({
            embeds: [format(lmsg.embed)]
...
module.exports = {
  ...
  usage: ["$testleave"],
  description: "test your servers leave message",
...
let variables = [
    ...
    */
    ...
    if (!data) {
      ...
      let lmsg = data.leave_message
      if(lmsg.embed_enabled == false && lmsg.text_enabled == false) return message.reply({content: "Leave Embed And Text Are Both Disabled"})
      if(lmsg.embed.title == null && lmsg.embed.description == null && lmsg.embed_enabled == true) return message.reply({content: "Leave Embed Title And Description Missing, One Is Required, Or Just Disable Leave Embed"})
      
      if(lmsg.text_enabled == true && lmsg.embed_enabled == true){
        message.channel.send({content: `${format(lmsg.content)}`, embeds: [format(lmsg.embed)]})
      } else if(lmsg.text_enabled == true && lmsg.embed_enabled == false){
        message.channel.send({content: `${format(lmsg.content)}`})
      } else if(lmsg.text_enabled == false && lmsg.embed_enabled == true){
        message.channel.send({embeds: [format(lmsg.embed)]})
      }
    } else {
      let channel = message.channel
      let lmsg = data.leave_message
