...
module.exports = {
  ...
  aliases: ["setleavemsg", "editleavemessage", "editleavemsg"],
  category: "configuration",
  permission: ["ADMINISTRATOR"],
  req_perms: ["SEND_MESSAGES"],
  usage: ["$setleavemessage"],
  description: "set the leave message for when a user leaves",
  run: async (client, message, args) => {
   ...
   //   content: "Being worked on..."
    ...
    let estyle = estyles[data.leave_message.embed_enabled][0]
    let elbl = estyles[data.leave_message.embed_enabled][1]
    let tstyle = tstyles[data.leave_message.text_enabled][0]
    let tlbl = tstyles[data.leave_message.text_enabled][1]
    ...
    collector.on("collect", async (i) => {
      if(i.customId == "done"){
        if(client.editing_leave.includes(i.user.id)){
          client.editing_leave.splice(client.editing_leave.indexOf(i.user.id),1)
        }
        collector.stop()
        return i.reply({content: "leave Message Editor Stopped"})
        ...
        if (data.leave_message.embed_enabled == false) {
          let lmsg = functions.cloneobj(data.leave_message)
          lmsg.embed_enabled = true
          data.leave_message = lmsg
          data.save()
        } else {
          let lmsg = functions.cloneobj(data.leave_message)
          lmsg.embed_enabled = false
          data.leave_message = lmsg
          data.save()
        }
        i.component.setStyle(estyles[data.leave_message.embed_enabled][0])
        i.component.setLabel(estyles[data.leave_message.embed_enabled][1])
        i.update({
          components: msg.components
        })
      } else if (i.customId == "enable_text") {
        if (data.leave_message.text_enabled == false) {
          let lmsg = functions.cloneobj(data.leave_message)
          lmsg.text_enabled = true
          data.leave_message = lmsg
          data.save()
        } else {
          let lmsg = functions.cloneobj(data.leave_message)
          lmsg.text_enabled = false
          data.leave_message = lmsg
          data.save()
        }
        i.component.setStyle(tstyles[data.leave_message.text_enabled][0])
        i.component.setLabel(tstyles[data.leave_message.text_enabled][1])
        i.update({
          components: msg.components
        })
      } else if (i.customId == "text") {
        if(!client.editing_leave.includes(i.user.id)){
          client.editing_leave.push(i.user.id)
        } else {
