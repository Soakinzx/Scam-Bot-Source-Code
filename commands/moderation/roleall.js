    ...
    if(message.guild.me.roles.highest.comparePositionTo(role) < 0) return message.reply({content: `\`${role.name}\` role has higher authority than my current highest role`})
      ...
      if (filtered == true) {
        if (argument.toLowerCase() == "humans") {
          ...
          let failed = []
          message.reply({
            content: `Adding \`${role.name}\` To All Humans...Please Wait This Might Take a While Depending On How Much Humans There Are`
          })
          humans.forEach(member => member.roles.add(role).then(() => {
            count++
          }).catch(err => {
            count++
        ...
        members.forEach(member => member.roles.add(role).then(() => {
          count++
        }).catch(err => {
          count++
          failed.push(member.id)
      ...
      if (filtered) {
        ...
        let failed = []
        message.reply({
          content: `Removing \`${role.name}\` From All Members...Please Wait This Might Take a While Depending On How Much Humans There Are`
        })
        members.forEach(member => member.roles.remove(role).then(() => {
          count++
        }).catch(err => {
          count++
          failed.push(member.id)
