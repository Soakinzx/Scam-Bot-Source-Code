...
module.exports = {
  ...
  category: "image",
  permission: [],
  usage: ["$ad <optional: user/attachment>"],
  req_perms: ["SEND_MESSAGES"],
  description: "ad user avatar/attachment",
  run: async (client, message, args) => {
    ...
    if (urls[0]) {
      let url = `https://api.popcat.xyz/ad?image=${urls[0].replace(".webp", ".jpg")}`
      let embed = {
        title: "AD",
        image: {
          url: url
      ...
      let url = `https://api.popcat.xyz/ad?image=${u.displayAvatarURL().replace(".webp", ".jpg")}`
      let embed = {
        title: "AD",
        image: {
          url: url
