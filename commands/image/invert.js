...
module.exports = {
  ...
  category: "image",
  permission: [],
  usage: ["$invert <optional: user/attachment>"],
  req_perms: ["SEND_MESSAGES"],
  description: "invert a user avatar/attachment",
  run: async (client, message, args) => {
    ...
    if (urls[0]) {
      let url = `https://api.popcat.xyz/invert?image=${urls[0].replace(".webp", ".jpg")}`
      let embed = {
        title: "Inverted",
        image: {
          url: url
      ...
      let av = u.displayAvatarURL().replaceAll(".webp", ".jpg")
      console.log(av)
      let url = `https://api.popcat.xyz/invert?image=${av}`
      let embed = {
        title: "Inverted",
        image: {
          url: url
