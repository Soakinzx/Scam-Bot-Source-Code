...
module.exports = {
  ...
  description: "shows all users with the same tag",
  usage: ["$tags <optional: user>"],
  run: async (client, message, args) => {
    if(!args[0]) {
        args[0] = message.author.id
    }
    let member = message.mentions.members.first() || message.guild.members.cache.get(args[0]) || message.guild.members.cache.find(m => m.user.username.toLowerCase().startsWith(args.join(" ").toLowerCase())) || message.guild.members.cache.find(m => m.user.tag.toLowerCase().startsWith(args.join(" ").toLowerCase())) || message.member
