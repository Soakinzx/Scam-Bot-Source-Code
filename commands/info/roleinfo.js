...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    let perms = role.permissions
      .toArray()
      .map((perm) => format(perm.replace(/_/g, " ")));
    let embed = new MessageEmbed()
      ...
      .addField(
        "Mentionable",
        functions.text_block(`${role.mentionable}`),
        true
      )
      ...
      .addField(
        "Created",
        functions.text_block(
          `${role.createdAt.toLocaleDateString()} | ${convert(
      ...
      .addField(
        "Permissions",
        functions.text_block(`${perms.length > 0 ? perms : "None"}`),
        true
      )
      .setFooter(`${role.id}`)
      .setTimestamp();
    ...
    if (role.unicodeEmoji != null) {
      embed.addField(
        "Emoji Unicode",
        functions.text_block(`${role.unicodeEmoji}`),
        true
      );
    }
    ...
    let members =
      role.members.map((m) => `\`${m.user.username}\``).join(", ").length > 4050
        ? role.members
            .map((m) => `\`${m.user.username}\``)
            .join(", ")
            .slice(0, 4050) + "...Too Many To Display"
        : role.members.map((m) => `\`${m.user.username}\``).join(", ");
    function inrole(i) {
      let embed = new MessageEmbed()
